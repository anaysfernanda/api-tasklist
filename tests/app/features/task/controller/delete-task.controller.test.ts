...
describe("Delete task controller tests", () => {
  beforeAll(async () => {
  ...
  test("Deveria retornar status 404 se o usuário não existir.", async () => {
    const res = await request(app).delete("/user/:userId/tasks/:taskId");
  ...
  test("Deveria retornar status 404 se a task não existir.", async () => {
    ...
    const res = await request(app).delete(`/user/${user.id}/tasks/:taskId}`);
  ...
  test("Deveria retornar status 200 se a task for excluida.", async () => {
    const user = await makeUser();
    ...
    const res = await request(app)
      .delete(`/user/${user.id}/tasks/${newTask.id}`)
      .expect(200);

    expect(res).toBeDefined();
    expect(res).toBeTruthy();
    expect(res.body.message).toEqual("Task excluída com sucesso!");
  });
